...
k =             getHessianKernels(s,numel(s),kernel_multiplier,1);
kp  = getHessianKernels_prototype(s,numel(s),kernel_multiplier,1);
kp2 = getHessianKernels_prototype(s,numel(s),kernel_multiplier,2);

linkedAxes3dO = plotKernels(k,'component','original');
linkedAxes3dP = plotKernels(kp,'component','protytpe');
linkedAxes3dP2 = plotKernels(kp2,'component','protytpe finite def');


function linkedAxes3d = plotKernels(kernels,element,supertitle)
if nargin < 2
    supertitle = '';
end
screenSize = get(0,'ScreenSize');
if contains(element,'norm')
    clims = [0 1];
else
...
%         clims = [kernels(i).min kernels.max]*.5; % this is incorrect
        imagesc(kernels.(element){i},clims);
        ax(i).XTick = (1:size(kernels.(element){i},2)+1)-1/2;
        ax(i).XTickLabel = {};
        ax(i).YTick = (1:size(kernels.(element){i},1)+1)-1/2;
        ax(i).YTickLabel = {};
        ax(i).TickDir = 'out';
    ...
    if ~strcmp(supertitle,''), fig.Name = [supertitle,', ',element]; end
elseif kernels.dims == 3
    ...
    for i = 1:kernels.dims^2
        ...
        imagesc(kernels.(element){i}(:,:,zslice),clims); hold on
        axSlices(i).XTick = (1:size(kernels.(element){i},2)+1)-1/2;
        axSlices(i).XTickLabel = {};
        axSlices(i).YTick = (1:size(kernels.(element){i},1)+1)-1/2;
    ...
    linkaxes(axSlices);
    if ~strcmp(supertitle,''), fig3.Name = [supertitle,', ',element]; end
    % 3d shapes
    ...
    for i = 1:kernels.dims^2
        ax3d(i) = nexttile;
        val = (max(kernels.(element){i}(:)) - min(kernels.(element){i}(:)))/2; % mean value for each component. This is to inspect the shape not to compare magnitudes
        p = patch(isosurface(kernels.(element){i},val));
        p.FaceColor = 'yellow';
        p.FaceAlpha = .3;
        p.EdgeColor = 'none';
        daspect([1 1 1])
        ...
        sslice = slice(kernels.(element){i}(:,:,:),xslice,yslice,zslice);
        set(sslice,'FaceAlpha',1,'FaceColor','flat','EdgeAlpha',0);
        sslice(end).FaceAlpha = 1;
        xlabel('x'); ylabel('y'); zlabel('z');
        ax3d(i).XLim = [1 kernels.nx];
        ax3d(i).XTick = (1:size(kernels.(element){i},2)+1)-1/2;
        ax3d(i).XTickLabel = {};
        ax3d(i).YLim = [1 kernels.ny];
        ax3d(i).YTick = (1:size(kernels.(element){i},1)+1)-1/2;
        ax3d(i).YTickLabel = {};
        ax3d(i).TickDir = 'out';
    ...
    if ~strcmp(supertitle,''), fig2.Name = [supertitle,', ',element];end
    rotate3d on
end
end