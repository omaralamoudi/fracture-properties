function hessianKernels = getHessianKernels(s,dims,m,implementation)
% GETHESSIANKERNELS input s should be a vector of length 2 or 3, e.g. [sx sy ...] where sx is
...
if implementation == 1
    ...
    if dims == 2 % 2d
        ...
        % initilizing H
        H = initH(x,dims);
        disp(['getHessianKernals 2d for s = [',num2str(s),']', ' kernal size = ' num2str([length(coord.X) length(coord.Y)])]);
    ...
    elseif dims == 3 % 3d
        ...
        % initilizing H
        H = initH(x,dims);
        disp(['getHessianKernals 3d for s = [',num2str(s),']', ' kernal size = ' num2str([length(coord.X) length(coord.Y) length(coord.Z)])]);
        ...
        error('getHessianKernels: physical dimension undetermined');
    end
elseif implementation == 2
    ...
    error('getHessianKernels: implementation undefined undetermined');
end
