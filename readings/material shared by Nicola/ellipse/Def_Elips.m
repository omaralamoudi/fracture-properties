...
function [Ar_p,a_p,b_p,Ar,xx_r,yy_r,sgm_1,sgm_2,tau_m,u_x_r,u_y_r,xi,nu,sgm_xi,sgm_nu]...
    =Def_Elips(plane_stress,sgm_11,sgm_22,v,mu,beta,alpha,plt)
% clear all
% close all
% clc
% format long e
...
if plt==1
    ...
    axes
    contourf(xx_r,yy_r,sqrt(sgm_1./sgm),16);
    set(gca,'position',[0.07 0.6 0.4 0.32],'xlim',[-2 2],'ylim',[-2 2]);
    title('\sigma_1/\sigma');
    shading flat
    colorbar
    axes
    contourf(xx_r,yy_r,sqrt(sgm_2./sgm),16);
    shading flat
    ...
    axes
    %ind     =   [1:3:numel(xx)];
    ind     =   [1+5:size(xx_r,1):numel(xx_r)-size(xx_r,1)+5];
    quiver(xx_r(ind),yy_r(ind),u_x_r(ind),u_y_r(ind));
    shading flat
    set(gca,'position',[0.57 0.09 0.4 0.4],'xlim',[-1 1],'ylim',[-1 1]);
    title('displacement');
end