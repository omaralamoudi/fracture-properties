...
\end{itemize}
\subsection[Orangi et al. (2011)]{\cite{Orangi2011}} 
...
\subsection[Lubbe and Worthington (2006)]{\cite{Lubbe2006}} 
...
\subsection[Hopkins et al. (1987)]{\cite{Hopkins1987}}
...
First, I use the command \texttt{tempcite\{\}\{\}} which produces \tempcite{temporary citation.}{Where this footnote provides additional information about the unformulated citation mentioned in the text, or an instruction to provide a citation about a specific topic discussed without proper references.} as a note that this is a temporary citation that needs further refinement, refer to the associated footnote for a further details. 

Second, I use the command \texttt{ilinstruction\{\}} which produces \ilinstruction{inline self-instructions within the text.} These inline self-instructions are meant to be intrusive and exist inline with the rest of the manuscript text to draw maximum attention. Other types of self-instructions commands that I use are \texttt{spinstruction\{\}\{\}} which produces \spinstruction{specific instruction}{Where more details are provided here}, where the second argument is shown in the footnote, and the command \texttt{instruction\{\}} which produces margin instructions that are associated with the following asterisks \instruction{This is an example of a margin instruction}, and some text in the margin that is vertically aligned with them. 

Third, I use the command \texttt{update\{\}} to \update{highlight} text that needs further updating. 

Finally, I use the command \texttt{moredetails\{\}} that produces a style that changes the color of some text, \moredetails{in order to highlight the need of more details.}
\end{document}
